...
export default function AddRoute() {
  ...
  const fetchBtrData = async (value: string) => {
    ...
    try {
      setSkeleton((prev) => ({ ...prev, btr: true }));
      ...
      setSkeleton((prev) => ({ ...prev, btr: false }));
    }
  };
  ...
  return (
    <>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-xl font-semibold">Add Allocate</h1>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow mb-6 p-6 grid md:grid-cols-3 gap-4">
        ...
        <InputFields
          ...
          value={form.btr}
          options={btrOptions}
          showSkeleton={skeleton.btr}
          disabled={!form.region_id || loadingBtr}
          error={errors.btr}
          onChange={(e) => handleChange("btr", e?.target?.value || "")}
        />

        <InputFields required label="Distributor" value={warehouseName} disabled showSkeleton={skeleton.warehouse} onChange={(e) => handleChange("warehouse_id", e?.target?.value || "")} error={errors.warehouse_id} />
        ...
        <InputFields required type="contact2"   label="Contact" value={form.contact} onChange={(e) => handleChange("contact", e.target.value)} error={errors.contact} />
      </div>
      ...
      <div className="flex justify-end gap-4 mt-6">
        ...
        <SidebarBtn label={submitting ? "Submitting..." : "Submit"} isActive onClick={handleSubmit} disabled={submitting} leadingIcon="mdi:check"/>
      </div>
    </>
  );
}
