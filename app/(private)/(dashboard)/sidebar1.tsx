...
export default function Sidebar({
  ...
  return (<>
    {/* hamburger icon */}
    <div className={`absolute top-1 opacity-80 rounded-full p-[10px] w-10 h-10 bg-[#121D33] text-white flex justify-center items-center z-50 sm:hidden ${showSidebar ? "rounded-r-none right-0" : "rounded-r-full left-1"}`}>
      <Icon
        icon={showSidebar ? "heroicons-outline:x-mark" : "heroicons-outline:menu-alt-1"}
        width={20}
        onClick={() => {
          setShowSidebar(!showSidebar);
          setIsOpen(showSidebar ? false : true);
          setCurrentPageForSecondSidebar(data[0].data[0].label);
          setSecondSidebarChildren(data[0].data[0].children ?? []);
          // setOpenSubMenus({});
        }}
    ...
    <div className={`${showSidebar ? "flex" : "hidden sm:flex"}`} ref={wrapperRef}>
      {/* first side bar */}
          ...
          <div className="flex flex-col items-center gap-[8px] w-full">
            {data.map((group, groupIndex) =>
              group.data.map((link, index) => {
                ...
                const isCurrentPageActive = currentPageForSecondSidebar === link.label && link.children && link.children.length > 0;
                return (
                  <Link
                    className={`w-full relative cursor-pointer group ${isActive && !isCurrentPageActive ? "p-1" : ""}`}
                    key={`${groupIndex}-${index}`}
                    href={link.href}
                    onClick={() => {
                      onClickHandler(link.href);
                      if (link.href !== "" && link.href !== "#") setActiveHref(link.href);
                      if (link.children && link.children.length > 0) {
                        setIsOpen(true);
                        setCurrentPageForSecondSidebar(link.label);
                  ...
                  >
                    <div
                      className={`w-full h-[40px] p-[6px] flex justify-center items-center relative ${isActive && isCurrentPageActive ? "bg-[#223458] rounded-l-[8px]"
                        : isActive ? "bg-[#223458] rounded-md"
                          : isCurrentPageActive ? "bg-[#223458] rounded-l-[8px]"
                            : ""
                        }`}
                    >
                      <Icon
                      ...
                      />
                      <div className={`z-10 ${isActive && isCurrentPageActive ? "block" : isCurrentPageActive ? "block" : "hidden"}`}>
                      ...
                      <div
                        className={`${!isActive ? "group-hover:flex" : ""
