...
import { migrateToSecureStorage } from "@/app/utils/secureStorageMigration";
import ThemePaletteModal from "@/app/components/ThemePaletteModal";
...
function LayoutSelector({ children }: { children: React.ReactNode }) {
    ...
    // Track the last pathname to detect URL changes - optimized for speed
    useEffect(() => {
        if (loading) return;

        const internalNav = sessionStorage.getItem('internal_nav');

        // Fast check: if not marked as internal and not profile page, redirect immediately
        if (internalNav !== 'true' && pathname !== '/profile') {
            const storedPath = sessionStorage.getItem('last_path');
            if (storedPath && storedPath !== pathname) {
                // Immediate redirect without additional checks
                router.replace('/profile');
                return;
            }
        }

        // Update stored path and clear flag in one operation
    ...
    // Mark all legitimate navigations as internal - optimized
    useEffect(() => {
        ...
        // Capture phase for immediate marking
    ...
    return loading ? <Loading /> : (
        <>
            {process.env.NODE_ENV === "development" && <ThemePaletteModal />}
            {theme === "layoutTheme" ? (
