...
import { getWarehouseById, getCustomerInWarehouse, getRouteInWarehouse, getVehicleInWarehouse, getSalesmanInWarehouse, getStockOfWarehouse, warehouseSales, downloadFile,returnByWarehouse ,exportReturnByWarehouse,allInvoiceExportInWarehouse, warhouseStocksByFilter, downloadFileGlobal } from "@/app/services/allApi";
        ...
        const exportSalesFile = async (uuid: string, format: string) => {
            ...
            const downloadPdf = async (uuid: string, code: string) => {
                try {
                    ...
                    if (response && typeof response === 'object' && response.download_url) {
                        await downloadFileGlobal(response.download_url, `Return - ${code}.pdf`);
                        // await downloadFile(response.download_url);
                        showSnackbar("File downloaded successfully ", "success");
                    } else {
                        showSnackbar("Failed to get download URL", "error");
            ...
            const downloadPdfofInvoice = async (uuid: string, code: string) => {
                try {
                    setLoading(true);
                    const response = await exportOrderInvoice({ uuid: uuid, format: "pdf" });
                    if (response && typeof response === 'object' && response.download_url) {
                        await downloadFileGlobal(response.download_url, `Invoice - ${code}`);
                        showSnackbar("File downloaded successfully ", "success");
                    } else {
                        showSnackbar("Failed to get download URL", "error");
                    }
                ...
                <div className="flex flex-col h-full">
                    <Table
                       ...
                       
                       {
                            ...
                            showLoading: true,
                            onClick: (row: TableDataType) => downloadPdfofInvoice(row.uuid,row.invoice_code),
                        },
                    ],
                            footer: { nextPrevBtn: true, pagination: true },
