    ...
    if (uuid) {
      (async () => {
        try {
          ...
          setReturnCode(data.osa_code || "");
          // Map details to table data
          if (data?.details && Array.isArray(data.details)) {
            const mappedData = data.details.map((detail: InvoiceData["details"][number], index: number) => ({
              id: String(index + 1),
              // itemCode: String(detail.detail.item_code ?? "-" ?? "-"),
              itemName: `${String(detail.item_code ?? "")} - ${String(detail.item_name ?? "")}`,
    ...
    const downloadPdf = async () => {
        try {
            ...
            if (response && typeof response === 'object' && response.download_url) {
                const fileName = `Return - ${returnCode}`;
