...
const columns = [
  ...
  { key: "total", label: "Amount", showByDefault: true, render: (row: TableDataType) => <>{toInternationalNumber(row.total) || '0.00'}</> },
  {
    key: "status",
    label: "Status",
    showByDefault: true,
...
export default function CustomerInvoicePage() {
  ...
  const exportFile = async (format: "csv" | "xlsx" = "csv") => {
    try {
      ...
      const response = await deliveryExportHeader({ format });
