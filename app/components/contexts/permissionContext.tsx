...
import { secureAuth } from "@/app/utils/secureStorage";
...
export function filterMenuByPermissions(allMenus: LinkDataType[] = [], role: any): LinkDataType[] {
  ...
  // Recursive function to filter nested children
  const filterChildren = (children: LinkDataType[]): LinkDataType[] => {
    return children
      ...
      .map((child: any) => {
        ...
        if (child.children && child.children.length > 0) {
          ...
          // Only include the parent if it has allowed children or is itself allowed
          if (nestedChildren.length > 0) {
            filteredChild.children = nestedChildren;
          } else if (!allowedSubMenus.includes(child.label)) {
            // If no children and parent itself is not allowed, return null
            return null;
          }
        }
...
export async function fetchFilteredMenu(allMenus: LinkDataType[] | null, type: string, roleId?: string): Promise<LinkDataType[]> {
  try {
    const rid = roleId ?? (typeof window !== "undefined" ? secureAuth.getRole() : null);
    if (!rid) return [];
...
export const PermissionProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const refreshPermissions = useCallback(
    async (allMenus: LinkDataType[] | null, type: string, options?: { roleId?: string; force?: boolean }) => {
      const roleId = options?.roleId ?? (typeof window !== "undefined" ? secureAuth.getRole() : null);
