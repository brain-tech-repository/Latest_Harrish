...
export default function FilterDropdown({
  ...
  // Compute fixed placement similar to AutoSuggestion: measure anchor and dropdown,
  // support align modes and table bounds, and recompute on resize/scroll.
  useLayoutEffect(() => {
    const compute = () => {
      ...
      const table = anchor.closest("table") as HTMLElement | null;
      ...
      if (align === "center") {
        ...
        const th = anchor.closest("th") as HTMLElement | null;
        if (th) {
    ...
    // initial compute
    compute();
    // recompute on resize and scroll so dropdown follows the page
  ...
  return (
    ...
    >
      <CustomDropdown>
        {/* inner scroll container: this will be the scrolling block for which sticky works */}
        <div
          className="flex flex-col"
          style={{
            height: dimensions?.height ?? (style.maxHeight as string) ?? '300px',
