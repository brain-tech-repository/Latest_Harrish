 ...
 * 
 * // For CSV
 * await downloadFileGlobal(response.download_url, 'export-data.csv');
 */
export async function downloadFileGlobal(
  ...
  try {
    ...
    if (useProxy) {
      try {
        window.location.href = `/api/force-download?url=${encodeURIComponent(url)}&filename=${encodeURIComponent(finalFileName)}`;
        return true;
      } catch (proxyError) {
...
/**
 * Download PDF with custom filename support
 * 
 * @param url - PDF download URL
 * @param fileName - Custom filename (with or without .pdf extension). If not provided, uses 'document.pdf'
 * @returns Promise<boolean>
 * 
 * @example
 * // Order module
 * await downloadPDFGlobal(response.download_url, 'order-invoice-123');
 * 
 * // Sales module
 * await downloadPDFGlobal(response.download_url, 'sales-report-2024');
 * 
 * // Return module
 * await downloadPDFGlobal(response.download_url, 'return-note-456');
 * 
 * // With .pdf extension already included
 * await downloadPDFGlobal(response.download_url, 'purchase-order.pdf');
 */
...
export const getRoleById = async (id: string, params?: Params) => {
  try {
    ...
    const res = await API.get(`/api/settings/roles/get-profile`, { params });
    return res.data;
  } catch (error: unknown) {
    return handleError(error);
  }
